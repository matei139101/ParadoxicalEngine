...
impl BaseController {
    pub fn new(name: String, transform: Transform, player_id: u8) -> BaseController {
        BaseController {
            name,
            transform,
            player_id,
    ...
    fn update(services: &Services, repositories: &Repositories) {
        if let Some(player_entity_id) = repositories
            .get_entity_repository()
            .get_player_controller(services.get_gamestate_service().get_active_player())
        {
            if let Some(mut transform) = repositories
                .get_entity_repository()
                .get_transform(player_entity_id)
            {
                ...
                {
                    transform.position =
                        transform.get_position() + transform.forward() * -input_forward as f32;
                } else {
                    log!(Self, Critical, "Failed to update controller transform...");
                }
                ...
                if let Some(input_right) = repositories.get_input_repository().get_axis("RIGHT") {
                    transform.position =
                        transform.get_position() + transform.right() * -input_right as f32;
                } else {
                    log!(Self, Critical, "Failed to update controller transform...");
                }
